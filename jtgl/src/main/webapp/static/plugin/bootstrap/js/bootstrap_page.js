...
function createPageNav(opt) {
    ...
    var $container   = opt.$container          || null, //必需，页码容器，请确保这个容器只用来存放页码导航
        pageCount    = Number(opt.pageCount)   || 1,    //必需，页码总数
        currentNum   = Number(opt.currentNum)  || 1,    //选填，当前页码
        maxCommonLen = Number(opt.maxCommonLen)|| 10,   //选填，普通页码的最大个数
        totalCount = Number(opt.totalCount)|| 0,  //选填 总数量
        pageSize = Number(opt.pageSize)|| 10,  //选填 每页显示多少条
    ...
    function _initPageNav(){
        ...
        if(hasInput)   
            initStr.push('<div class="input-page-div">当前第<input type="text" id="__page"  style="width:25px;background-color: #ffffff;background-image: none;border: 1px solid #e7e7e2;border-radius: 4px;box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075)" value="' + currentNum + '" />页 / 每页<input type="text" id="__pageSize"  style="width:25px;background-color: #ffffff;background-image: none;border: 1px solid #e7e7e2;border-radius: 4px;box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075)" value="' + pageSize + '" />条，共'
                + totalCount+'条<button type="button" class="btn btn-xs input-btn-xs" style="background-color:#75b9e6">确定</button></div>');
        initStr.push('</ul></nav>');
        ...
        if(hasInput){
            ...
            $parent.find("div.input-page-div>button").click(function(){
                _gotoPage($input.val(),$("#__pageSize").val());
            });
        }    
        //初始化功能按钮
        ...
        $parent.on("click", "li",function () {
            ...
            if ($this.is("li") && $this.attr("value")){
                if(!$this.hasClass("disabled") && !$this.hasClass("active")){
                    _gotoPage($this.attr("value"),$("#__pageSize").val());
                }
            }
        });
    }
    //跳转到页码
    function _gotoPage(targetNum,__pageSize) {
        targetNum=_formatNum(targetNum);
        if (targetNum == 0) return false;
        // 跳转前回调函数
        if (beforeFun && beforeFun(targetNum,__pageSize) === false) return false;
        //修改值
