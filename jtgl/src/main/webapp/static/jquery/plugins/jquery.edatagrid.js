...
(function($){
	...
	function setEditorInitValue(target){
		...
		if (editors.length){
			for (var i=0;i<editors.length;i++){
				var editor_type = editors[i].type;
				var editor_field = editors[i].field;
				var t = editors[i].target;
				//t.selectAll();
				if(editor_field.indexOf('.') != -1 ){
					var strFields=editor_field.split(".");
					if(strFields.length==2 && row[strFields[0]]!= undefined){
						value = row[strFields[0]][strFields[1]];
					} else if(strFields.length==3 && row[strFields[0]]!= undefined&& row[strFields[0]][strFields[1]]!= undefined){
						value = row[strFields[0]][strFields[1]][strFields[2]];
					} else {
						value='';
					}
					
				} else {
					value = row[editor_field];
				}
				
				if( editor_type == 'combobox' || editor_type == 'combogrid'){
					if( editor_type == 'combobox'){
						t.combobox('setValue',value);
					} else if (editor_type == 'combogrid'){
				...
				if ( editor_type == 'checkbox'){
                   ...
                   //t.attr("onclick",editor_field);
				}
				
				if ( editor_type == 'datebox'){
					//t.val(formateDateColumn(value));
				}
				
				if ( editor_type == 'my97'){
					
                    t.attr("onclick","");
                    t.unbind("click.myNameSpace").bind("click.myNameSpace",function(e){
                        var initObj = {dateFmt:'yyyy-MM-dd',readOnly:true};
                        if(value != "") initObj["startDate"] = t.val();
                        WdatePicker(initObj);
                    });
				}
			}
