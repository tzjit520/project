	...
	<div class="content-wrapper">
		<div class="outlet">
			<div class="col-xs-12 heading">
				<h1 class="page-header">序列管理</h1><hr/>
			</div>
			<div class="panel-body">
				<form:form modelAttribute="sequence" class="form-horizontal" method="get" role="form" id="seachForm" cssStyle="padding-bottom:20px;">
					...
					<div class="form-group">
						...
						<div class="col-xs-5">
							...
							<shiro:hasPermission name="sequence:add">
								<input type="button" id="btnAdd" class="btn btn-success" onclick="window.location.href='${ctx}/api/v1/sequence/0'" value="添加">
							</shiro:hasPermission>
							<shiro:hasPermission name="sequence:update">
				...
				<table class="table table-bordered">
					...
					<tbody>
						<c:forEach items="${page.data}" var="sequence">
							<tr>
								<td>
									<input type="checkbox" value="${sequence.id}" name="child_items">
								</td>
								<td>${sequence.code}</td>
								<td>${sequence.name}</td>
								<td>${sequence.formartStr}</td>
								<td>${sequence.seqNumber}</td>
								<td>${sequence.seqStep}</td>
								<td>${sequence.remark}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
	...
	<script type="text/javascript">
	    ...
	    $('#btnUpdate').click(function() {
			...
			if(node_count <= 0){
				...
				window.location.href = "${ctx}/api/v1/sequence/"+id;
			}
		});
		...
		function page(num, pageSize){
			...
			$("#seachForm").attr("action","${ctx}/api/v1/sequence");
			$("#seachForm").submit();
		}
    </script>
</body>
