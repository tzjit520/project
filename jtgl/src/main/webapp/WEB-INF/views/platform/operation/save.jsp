	...
	<div class="content-wrapper">
		<div class="outlet">
			<div class="col-xs-12 heading">
				<h1 class="page-header">${empty operation.id?'添加':'修改'}操作</h1><hr/>
			</div>
			<div class="panel-body">
				<form:form modelAttribute="operation" action="${ctx}/api/v1/operation" role="form" id="operationForm" onsubmit="return submitForm()" class="form-horizontal group-border hover-stripped">
					<form:hidden path="id" id="id" />
					<input type="hidden" name="_method" value="${empty operation.id?'POST':'PUT'}">
					...
					<div class="form-group">
						...
						<div class="col-xs-2">
							...
							<button class="btn btn-info" type="button" onclick="window.location.href='${ctx}/api/v1/operation'">返回</button>
						</div>
					</div>
				</form:form>
			</div>
	    ...
	    $(document).ready(function() {
	    	$("#operationForm").validate({
	    		...
	    		rules: {
	    			code: {
	    				required: true,
	    				remote: "${ctx}/api/v1/operation/unique?id=${operation.id}"
	    			},
	    			name: {
	    				required: true,
	    				remote: "${ctx}/api/v1/operation/unique?id=${operation.id}"
