	...
	<div class="content-wrapper">
		<div class="outlet">
			<div class="col-xs-12 heading">
				<h1 class="page-header">对象管理</h1><hr/>
			</div>
			<div class="panel-body">
				<form:form modelAttribute="dbobject" class="form-horizontal" method="get" role="form" id="seachForm" cssStyle="padding-bottom:20px;">
					...
					<div class="form-group">
						...
						<div class="col-xs-5">
							...
							<shiro:hasPermission name="dbobject:add">
								<input type="button" id="btnAdd" class="btn btn-success" onclick="window.location.href='${ctx}/api/v1/dbobject/0'" value="添加">
							</shiro:hasPermission>
							<shiro:hasPermission name="dbobject:update">
								<input type="button" id="btnUpdate" class="btn btn-purple" value="修改">
							</shiro:hasPermission>
							<shiro:hasPermission name="dbobject:delete">
				...
				<table class="table table-bordered">
					...
					<tbody>
						<c:forEach items="${page.data}" var="dbobject">
							<tr>
								<td>
									<input type="checkbox" value="${dbobject.id}" name="child_items">
								</td>
								<td>${dbobject.name}</td>
								<td>${dbobject.code}</td>
								<td>${dbobject.sqlStatements}</td>
								<td>${dbobject.remark}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
	...
	<script type="text/javascript">
	    ...
	    $('#btnUpdate').click(function() {
			...
			if(node_count <= 0){
				...
				window.location.href = "${ctx}/api/v1/dbobject/"+id;
			}
		});
		...
		function page(num, pageSize){
			...
			$("#seachForm").attr("action","${ctx}/api/v1/dbobject");
			$("#seachForm").submit();
		}
    </script>
</body>
