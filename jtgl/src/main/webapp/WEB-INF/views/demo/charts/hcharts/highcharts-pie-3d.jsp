		...
		chart = new Highcharts.Chart({
	        chart: {
	            ...
	            options3d: {//3D图像设置项。3D效果需要引入highcharts-3d.js
	                ...
	                alpha: 50,//3D图旋转角度，此为α角，内旋角度 默认是：0
	                beta: 0,//3D图旋转角度，此为β角，外旋角度 默认是：0
	        ...
	        legend: {  //图例 
	            layout: 'horizontal',  //String 图例数据项的布局。布局类型：水平或垂直。默认是：水平 (horizontal)   水平（horizontal）/垂直（vertical） 
	            backgroundColor: '#ffc', //Color 图例容器的背景颜色
	            align: 'center', //String 图例容器（中的图例）水平对齐在图表区，合法值有"left", "center" 和 "right". 默认是： center. 默认是：center
	            ...
	            verticalAlign: 'bottom',//String 垂直对齐。能取"top", "middle" or "bottom"之一。垂直对齐的位置可通过Y设置进一步调整它的位置。 默认是：bottom
	            x: 40,//Number 整个图例Ｘ轴偏移量，它是相对于水平布局定下后，chart.spacingLeft 和 chart.spacingRight.的空间左右移动，当ｘ值为负值时，图例朝左移动；正值时，图例朝右移动。 默认是：0
	            y: 0//Number 整个图例垂直偏移量，它是相对于垂直布局定下后，chart.spacingTop 和 chart.spacingBottom的空间垂直移动，当y值为负值时，图例朝上移动；正值时，图例朝下移动。 默认是：0
	        },
	        plotOptions: {
	        	pie: {
