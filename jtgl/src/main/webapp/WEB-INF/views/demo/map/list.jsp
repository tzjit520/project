	...
	<div class="content-wrapper">
		<div class="outlet">
			<div class="col-xs-12 heading">
				<h1 class="page-header">地图数据</h1><hr/>
			</div>
			<div class="panel-body">
				<form:form modelAttribute="map" class="form-horizontal group-border hover-stripped" method="get" role="form" id="seachForm" cssStyle="margin-bottom:20px;">
					...
					<div class="form-group">
						...
						<div class="col-xs-8">
							...
							<input type="button" id="btnAdd" class="btn btn-success" onclick="window.location.href='${ctx}/api/v1/map/0'" value="添加">
				...
				<table class="table table-bordered table-hover table-striped">
					...
					<tbody>
						<c:forEach items="${page.data}" var="m">
							<tr>
								<td>
									<input type="checkbox" value="${m.id}" name="child_items">
								</td>
								<td>${m.name}</td>
								<td>${m.number}</td>
								<td>${m.mapType == 1 ? '百度地图' : '高德地图'}</td>
								<td>${m.province}</td>
								<td>${m.city}</td>
								<td>${m.area}</td>
								<td>${m.address}</td>
	...
	<script type="text/javascript">
		...
		$('#btnUpdate').click(function() {
			...
			if (node_count <= 0) {
				...
				window.location.href = "${ctx}/api/v1/map/" + id;
			}
		});
		...
		function page(num, pageSize) {
			...
			$("#seachForm").attr("action", "${ctx}/api/v1/map");
			$("#seachForm").submit();
		}
	</script>
</body>
