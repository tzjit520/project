...
<mapper namespace="${package}.mapper.${ClassName}Mapper">
    ...
    <select id="selectByPrimaryKey" parameterType="java.lang.Long" resultMap="BaseResultMap">
  	...
  	<select id="selectByProperties" parameterType="${package}.model.${ClassName}" resultMap="BaseResultMap">
   		select <include refid="Base_Column_List_Alias" />	from ${tableName} ${className}
    	where 1=1 and ${className}.deleted = 0
<#if (columns?exists) && (columns?size>0)>
<#list columns as column>
	<if test="${column.attrName} != null">
		<#if (column.attrType == 'String')>
		AND ${className}.${column.columnName} like '%'||${r"#{"}${column.attrName}${r"}"}||'%'
		<#else>
  	...
  	<delete id="deleteByPrimaryKey" parameterType="java.lang.Long">
	    delete from ${tableName} where id = ${r"#{id, jdbcType=DECIMAL}"}
  	</delete>
  	<update id="logicalDeleteByPrimaryKey" parameterType="java.lang.Long">
	    update ${tableName} set deleted = 1 where id = ${r"#{id, jdbcType=DECIMAL}"}
  	</update>
